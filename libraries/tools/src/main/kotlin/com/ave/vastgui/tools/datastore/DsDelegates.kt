...
// Date: 2023/5/29
// Documentation: https://sakurajimamaii.github.io/AVE-DOC/documents/tools/architecture-components/data-layer-libraries/datastore/
...
/**
 * [IDataStoreOwner]. It holds [DataStore] and provides some expansion
 * methods.
 *
 * ```kotlin
 * // Here is a example
...
abstract class DataStorePreference<V> internal constructor() : IDataStorePreference<V> {
    /**
     * Save value into [dataStore], or remove the preferences from [dataStore]
    ...
    /**
     * Returns the first element emitted by the [asFlow] and then cancels
     * flow's collection
     *
     * @since 0.5.6
     */
    ...
    fun asNotNullFlow(): Flow<V> =
        dataStore().data.map {
            it[key()] ?: default() ?: throw RuntimeException("The value of ${key().name} is null.")
        }
