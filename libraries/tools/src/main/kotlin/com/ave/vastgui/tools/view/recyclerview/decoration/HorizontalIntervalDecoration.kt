...
/**
 * Horizontal [RecyclerView.ItemDecoration].
 *
 * @since 1.5.2
 */
class HorizontalIntervalDecoration : RecyclerView.ItemDecoration {
    ...
    constructor(space: Float, setLeftAndRight: Boolean) : super() {
        this.space = space.coerceAtLeast(0f).roundToInt()
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (manager.orientation == RecyclerView.HORIZONTAL) {
            if (position == 0) {
                outRect.left = if (setLeftAndRight) space else 0
                outRect.right = space / 2
            } else if (position == totalCount - 1) {
                outRect.left = space / 2
                outRect.right = if (setLeftAndRight) space else 0
            } else {
                outRect.left = space / 2
                outRect.right = space / 2
            }
