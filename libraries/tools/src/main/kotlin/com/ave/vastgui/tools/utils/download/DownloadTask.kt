...
class DownloadTask internal constructor(
    ...
    private val handler = CoroutineExceptionHandler { _, cause ->
        Log.d("Test", "onFailure ${Thread.currentThread() == Looper.getMainLooper().thread}")
        listener?.onCancel()
    }
    ...
    internal fun onDownload() {
        if (subTasks.all { it.isCancelled }) return
    ...
    fun pause() {
        launch {
            if (isValidEvent(DownloadEvent.Pause)) {
                _eventFlow.emit(currentEvent)
            }
        }
    }
    ...
    fun resume() {
        launch {
            if (isValidEvent(DownloadEvent.Resume)) {
                _eventFlow.emit(currentEvent)
            }
        }
    }
    ...
    private fun isValidEvent(event: DownloadEvent): Boolean =
        (currentEvent <= event).also { currentEvent = event }
