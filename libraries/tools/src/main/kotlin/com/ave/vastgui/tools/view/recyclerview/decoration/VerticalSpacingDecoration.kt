...
/**
 * Vertical spacing decoration of [RecyclerView].
 *
 * @since 1.5.2
 */
class VerticalSpacingDecoration : RecyclerView.ItemDecoration {
     ...
     * @since 1.5.2
     */
    val setTopAndBottom: Boolean

    /**
     * @param spacePx Refer to [VerticalSpacingDecoration.spacePx].
     * @since 1.5.2
     */
    constructor(spacePx: Float) {
        this.spacePx = spacePx
        setTopAndBottom = false
    }

    /**
     * @param spacePx Refer to [VerticalSpacingDecoration.spacePx].
     * @param setTopAndBottom Refer to
     * [VerticalSpacingDecoration.setTopAndBottom].
     * @since 1.5.2
     */
    constructor(spacePx: Float, setTopAndBottom: Boolean) {
        this.spacePx = spacePx
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (manager.orientation == RecyclerView.VERTICAL) {
            when (position) {
                0 -> {
                    outRect.top = if (setTopAndBottom) spacePx.roundToInt() else 0
                    outRect.bottom = (spacePx / 2f).roundToInt()
                }

                totalCount - 1 -> {
                    outRect.top = (spacePx / 2f).roundToInt()
                    outRect.bottom = if (setTopAndBottom) spacePx.roundToInt() else 0
                }
            ...
            outRect.top = spacePx.roundToInt()
            outRect.bottom = spacePx.roundToInt()
        }
    }
}