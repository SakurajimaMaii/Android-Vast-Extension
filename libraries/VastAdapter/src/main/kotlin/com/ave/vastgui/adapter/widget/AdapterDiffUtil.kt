...
/**
 * Because of the use of [AdapterItemWrapper], the original [DiffUtil]
 * will be inconvenient to use, so [AdapterDiffUtil] is designed to replace it.
 *
 * @param T The class of the list item.
 */
abstract class AdapterDiffUtil<T> : DiffUtil.ItemCallback<AdapterItemWrapper<T>>() {

    /**
     * @see newAreContentsTheSame
     */
    final override fun areContentsTheSame(
        oldItem: AdapterItemWrapper<T>,
        newItem: AdapterItemWrapper<T>
    ): Boolean {
